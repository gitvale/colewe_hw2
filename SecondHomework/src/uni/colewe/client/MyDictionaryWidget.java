...
public class MyDictionaryWidget extends Composite {
	...
	public MyDictionaryWidget() {
		...
		chkSw.setText("Match words starting with query");
	...
	void onSearch(ClickEvent e) {
		dictService.queryServer(txtQuery.getText(), chkRev.getValue(), chkSw.getValue(),
				new AsyncCallback<List<MyDictionaryEntry>>() {

					public void onFailure(Throwable caught) {
						lblResponse.setText("Search failed");
	...
	private String highlight(String target) {
		if (chkSw.getValue()) {
			return target.replaceAll("(^|[\\s\\(\\)\\[\\]\\.,])(" + txtQuery.getText() + ")", "$1<mark>$2</mark>");
		} else {
